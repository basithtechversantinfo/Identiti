class SubmissionsAggregatorScoring
           ...
           [["data", ""]].to_h
            ...
            if get_drop_down_question_options(q)[:options].present?
             unless q.question_type == Question.question_types.key(12)
              if q.question_type == Question.question_types.key(3) 
                get_drop_down_question_options_with_scored(q)[:options].map{|shape|
            ...
            if submission.data_json["data"][cat].present?
              ...
              if submission.data_json["data"][cat][ques].is_a?(Array)
                submission.data_json["data"][cat][ques].each do |sub_data_a|
                  ...
                  # response[cat][ques][sub_data_a] << score_map[cat][ques][sub_data_a]
                  # response["recipients"][cat][ques][submission.id][submission.recipient_id] << score_map[cat][ques][sub_data_a]
                end
              else
                # if response[cat][ques].key?(submission.data_json["data"][cat][ques]).present?
                #   response[cat][ques][submission.data_json["data"][cat][ques]] << score_map[cat][ques][submission.data_json["data"][cat][ques]]
                #   response["recipients"][cat][ques][submission.id][submission.recipient_id] << score_map[cat][ques][submission.data_json["data"][cat][ques]]
                # else
                ...
                if response[cat][ques].key?(score_map[cat][ques].key(submission.data_json["data"][cat][ques])).present?
                  ...
                  if question.question_type == Question.question_types.key(3) && question.label_template_id != 0 && question.label_template_id != 10
                    response[cat][ques][submission.data_json["data"][cat][ques]] << score_map[cat][ques][submission.data_json["data"][cat][ques]]
                    response["recipients"][cat][ques][submission.id][submission.recipient_id] << score_map[cat][ques][submission.data_json["data"][cat][ques]]
                  else
                    response[cat][ques]["data"] << submission.data_json["data"][cat][ques]
