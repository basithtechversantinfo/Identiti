class Groups::SurveysController < ApplicationController
  before_action :set_group
  before_action :set_survey, only: [:results]
  ...
  def set_group
    @group = Group.where(archived: false).find(params[:group_id]) rescue nil
    @main_group = Group.where(archived: false).find(params[:group_id]) if params[:group_id].present? rescue nil
    if @group.nil?
  ...
  def set_survey
    @survey = @group.surveys.find(params[:id]) rescue nil
    if @survey.nil?
